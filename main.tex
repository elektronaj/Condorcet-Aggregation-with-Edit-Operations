...
\keywords{social choice; single-winner elections; multiwinner elections; participatory budgeting; participatory legislation}
...
Specifically, our model, described formally below, consists of two ingredients:
  (1) a \emph{ground set} $D$, which can be infinite, and contain all possible outcomes of the aggregation process
  and
  (2) a set $E$ of \emph{edit operations}, each of which can be applied to any element of $D$ and results in transforming the element to a different element in $D$.
Given these two ingredients, we associate each voter with a single element of $D$, referred to as the voter \emph{ideal point}, and understood as her most-preferred element from $D$.
Then we use the fact that the set $E$ defines a metric over the elements of $D$ to infer, for each voter, a ranking over all the elements of $D$, by assuming that a voter prefers closer elements to elements which are farther away from her ideal point (i.e., from elements which require more expensive applications of edit operations).

Given some element space $D$ and $n$ ideal points in it, representing $n$ voters, we are interested in aggregating the $n$ inferred rankings. Specifically, we concentrate on Condorcet-consistent aggregation methods:
...
We show that, if certain conditions hold on the set $D$ and the set of edit operations $E$,
...
by defining, for each such setting, what is the ground set $D$ and what is the set of edit operations $E$.
